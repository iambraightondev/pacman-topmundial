...
/* ---- las piezas nuevas ---- */
var skins  = leer('a-skins.js') + '\n' + leer('a-skins2.js');
var resto  = leer('a-resto.js');
var muertes = leer('a-muertes.js');
var cat    = leer('a-cat.js');

/* Adaptadores: el resto está escrito como lo quiere el JUEGO (ACC.id, EFX.id,
 * caraAndina) y la vitrina pinta por DRAW.id. Se enchufan aquí, que es de una vez
...
var puente = [
  ...
  '  });',
  "  ['achachau', 'huayno', 'chevere', 'chau', 'rico'].forEach(function (id) {",
  "    DRAW['emo_' + id] = function (ctx, o) {",
  '      body(ctx, o);',
  '      globoEmote(ctx, o.x, o.y - 11, o.c, o.t * 60, function (cx, cy, r) {',
  '        caraAndina(ctx, cx, cy, r, o.c, id, o.t * 60);',
  '      });',
  '    };',
  '  });',
  ''
...
salida = salida
  .replace('<title>Vitrina de skins Top Mundial</title>',
           '<title>Vitrina andina Top Mundial</title>')
  .replace('<h1>Tanda extravagante</h1>', '<h1>Liga andina</h1>')
  .replace(/<p class="lede">[\s\S]*?<\/p>/, LEDE)
  ...
  .replace(/<p>Las ocho son extravagantes[\s\S]*?<\/p>/,
    '<p>El gallito de las rocas, un puma tallado en piedra, una papa, un ají, el sapo de bronce de la cantina, el tumi de oro, el sol Inti y el colibrí de Nazca. Cinco se compran; tres solo salen de un cofre.</p>');
